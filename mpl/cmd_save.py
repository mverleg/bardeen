...
def order_imgs(filenames, img_map, mpl = None, properties = {}):
	"""
		...
		where function can be either the name of the function in the module, or any python function
			...
			IMGMAP = {
				('test', 'test2'): ('demo_image', 'demos', lambda **kwargs: demos.generate_demos(**kwargs), {}),
				...
			}

		:param mpl: optionally, an mpl instance
		:param properties: optionally, a list of properties to use for orders

		exits with status code 1 if not all images were ordered; some stdout and stderr messages
	"""
	...
	for files, (label, pypath, func, kwargs) in img_map.items():
		...
		if matches:
			...
			module = __import__(pypath, fromlist = [pypath]) # <-- why fromlist? (it imports A instead of B for
			                                                 # import A.B without fromlist, but why?)
			if func is None:
...
def from_argv(img_map, mpl = None, properties = {}):
	return order_imgs(argv[1:], img_map, mpl = mpl, properties = properties)
